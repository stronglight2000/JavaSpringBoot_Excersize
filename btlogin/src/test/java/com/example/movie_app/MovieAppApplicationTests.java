...
class MovieAppApplicationTests {
    ...
    void save_favourites() {
        ...
        for (int i = 0; i < new Random().nextInt(11) + 50; i++) {
            Movie movie = movies.get(new Random().nextInt(movies.size()));

            // Kiểm tra xem đã tồn tại favourite chưa
            boolean isExist = favouriteRepository.existsByUser_IdAndMovie_Id(user.getId(), movie.getId());
            if (!isExist) {
